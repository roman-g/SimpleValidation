...
namespace SimpleValidation.Core.Tests
{
	public class MemberHelpersTest
	{
		...
		public void Simple()
		{
			...
			var fieldFail = builder.Member(x => x.StringField).Rule(Fail)(sample);
			fieldFail.IsFail.ShouldBeTrue();
			fieldFail.FailValue.ShouldBe((sample, "StringField", "field"));

			var propFail = builder.Member(x => x.StringProperty).Rule(Fail)(sample);
		...
		public void Predicate()
		{
			...
			var success = rule(sampleForSuccess);
		...
		private static ValidationResult<(TestClass, string, string)> Fail(MemberRuleContext<string, TestClass> context)
		{
			return ValidationResult<(
					...
					string memberName,
					string memberValue)>
				.Fail((context.Input, context.MemberName, context.MemberValue));
		}
